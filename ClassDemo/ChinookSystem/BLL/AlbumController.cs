...
namespace ChinookSystem.BLL
{
    ...
    public class AlbumController
    {
        ...
        public List<ArtistAlbumByReleaseYear> Albums_ListforArtist(int artistid)
        {
            using (var context = new ChinookContext())
            {
                var results = from x in context.Albums  
                              ...
                              {
                                  Title = x.Title,      //Title and Released become the properties of our class
        ...
        [DataObjectMethod(DataObjectMethodType.Select, false)]
        public List<Album> Albums_ListbyYearReleaseRange(int minyear, int maxyear)
        {
            using (var context = new ChinookContext())
            {
                var results = from x in context.Albums
        ...
        public int Albums_Add(Album item)
        {
            using (var context = new ChinookContext())
            {
                ...
                context.SaveChanges(); //actual commit of request
                return item.AlbumId;
            }
        }//eom
